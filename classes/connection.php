...
class dba
{
    ...
    function dba($dbuser, $dbpassword, $dbname, $dbhost)
    {
    ...
    function make_update($table_name, $array, $updated_key, $updated_id)
    {
        ...
        $sqlk = "";
        foreach ($array as $key => $value) {
            $sqlk .= $key . '=' . $this->qstr($value) . ' ,';
        }
        $s = $sql . ' ' . substr($sqlk, 0, strlen($sqlk) - 1) . ' where ' . $updated_key . ' = ' . $this->qstr($updated_id);
    ...
    {
        $category = array();
        while ($row = mysql_fetch_assoc($date_array)) {
    ...
    function get_table_by_id($query, $key, $val)
    {
        ...
        $result = array();
        $query = mysql_query("select * from " . $query) or die (mysql_error());
    ...
    function get_table($table_name)
    {
        ...
        $q = mysql_query("select * from " . $table_name) or die (mysql_error());
