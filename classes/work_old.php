...
class work
{
    ...
    public function add_operation_package($pack_id, $p_type = "1", $status = "1", $scratch = "0")
    {
        ...
        if (count($case) == "1") {
            ...
            for ($i = 0; $i < count($case); $i++) {
                ...
                $q = mysql_query("insert into orders_package_work (order_id,package_id,operation,status,p_type,scratche,start_time) 
				value ('" . $row['order_id'] . "','" . $row['id'] . "','" . ($case[$i] - 1) . "','" . $status . "','" . $p_type . "','" . $scratch . "','" . $date . "')");
            }
    ...
    {
        $query = mysql_query("select * from orders_image where order_id = '" . $order_id . "' ") or die(mysql_error());
    ...
    public function get_out_orders($finished, $status)
    {
        $query = mysql_query("select out_orders.*, account.account_company from  out_orders inner join account on (account.account_id = out_orders.account_id) where out_orders.status = '" . $status . "' and out_orders.finished = '" . $finished . "' order by id desc ");
    ...
    public function search_out_orders($out_order_id, $order_id)
    {
        ...
        if ($out_order_id != '') {
            $sql .= " and out_orders.id = '".$out_order_id."' ";
        }
        ...
        if ($order_id != '') {
            ...
            where orders_package.order_id = '".$order_id."' group by out_orders.id,account.account_company";
        }
    ...
    public function scratche_report($data)
    {
        ...
        package_type.package_name,
        concat(orders_package.order_id,'-',orders_package.part_order)
		from orders_scratch 
        ...
        if (count($data) > 0) {
            ...
            if (isset($data['from_date']) && $data['from_date'] != '') {
                $sql .= " orders_scratch.date_scrache >=  '" . $data['from_date'] . " 00:00:00' and";
            }
            if (isset($data['to_date']) && $data['to_date'] != '') {
                $sql .= " orders_scratch.date_scrache <  '" . $data['to_date'] . " 23:59:59' and";
            }
    ...
    public function get_uncompleted_order_package_details($date, $service_id = "", $package_type="")
    {
        $account_id="";
        if ($service_id!="" && $package_type !="") {
            $account_id="and orders.account_id='".$service_id."' and orders_package.package_type='".$package_type."'";
        } elseif ($service_id!="") {
            $account_id="and orders.account_id='".$service_id."'";
        } elseif ($package_type !="") {
        ...
        $query = mysql_query("select $sql  from orders_package_work $innerJoin where orders_package_work.order_id in (select orders.order_id from orders where orders.order_status = '1' and delivery_date <=' $date ' $account_id ) 
        and orders_package_work.status in (1,2) 
        and orders_package_work.operation not in (5,6) group by orders_package.id order by orders_package_work.order_id  ") or die(mysql_error());
