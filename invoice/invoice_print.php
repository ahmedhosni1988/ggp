...
//********************
$lineitem_hdr = array();
$lineitem_hdr[0] = "رقم التعريفى";
$lineitem_hdr[1] = "رقم القطعة";
$lineitem_hdr[2] = "التفاصيل";
$lineitem_hdr[3] = "&nbsp;&nbsp;سعر ";
...
$discount_value = sprintf("%01.2f",$invhdr["discount_value"]);
   ...
   if ($option_wait_extra_lines == 2)  {
    for ($i=1; $i<14; $i++)  {
     ...
     if ($li_waybills[$n+1] == $li_waybills[$n]  and  $pos > 0 ) {
      ...
      $line[3]           = sprintf("%01.2f",$li_amount[$n+1]);
...
showrow(  array($ttl,"",$total_fmt-$discount_value),          $inputfmnd_prt);
if ($invhdr["ppd_amount"] != 0)  {
