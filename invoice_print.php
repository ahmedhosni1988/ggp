...
//********************
$lineitem_hdr = array();
$lineitem_hdr[0] = "Reference";
$lineitem_hdr[1] = "Item";
$lineitem_hdr[2] = "Details";
$lineitem_hdr[3] = "&nbsp;&nbsp;Amount ";

if ($option_fs_on_lines == "Y") {
    $lineitem_hdr[4] = "Fuel<br>Surcharge";
    $lineitem_hdr[5] = "&nbsp;&nbsp;Total ";
}
...
//*header top left                                                             *
echo "  <td valign=top width=$htlwidth> <table border=$itborder cellspacing=$ospacing> ";
showrow(array("<IMG SRC=\"" . SITEURL . "/images/logo.jpg\" width=\"100\">"), $inputfmw_prt);
echo "      </table> </td>";
//*header top left                                                             *
...
//*header top right                                                            *
echo "<td   align=left valign=top width=$htrwidth> <table border=$itborder cellspacing=$htrspacing> ";
$iw = array();
$iw[0] = 250;
$bar = array();
$bar[0] = "امر تسليم";
...
$im = substr($idate, 5, 2);
$id = substr($idate, 8, 2);
$invoicedate_formatted = date(DATE_FORMAT, mktime(0, 0, 0, $im, $id, $iy));
...
//*header bottom left                                                          *
echo "  <td align=left valign=top width=$hblwidth> <table border=$itborder cellspacing=$hbspacing> ";
showrow(array("$bill_or_sold to: &nbsp;$acctno"), $inputfmnd_prt);
showrowbold(array($left_name), $inputfmw_prt);
showrowbold(array($left_addr), $inputfmw_prt);
showrowbold(array($left_city), $inputfmw_prt);
showrowbold(array($left_pcode), $inputfmw_prt);
if (trim($billto_apcontact <> "")) {
    showrow(array("Attn: " . $billto_apcontact), $inputfmw_prt);
    $extra_hdrlines = $extra_hdrlines + 1;
}
echo "      </table> </td>";
//*header bottom left                                                          *
...
if ($option_linedetail == 1) {
    ...
    for ($n = 1; $n < $showlines_count + 1; $n++) {
        ...
        $line[0] = "";
        $line[3] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_amount[$n]);
        //Reference
        ...
        //Delivery details
        if ($li_chgtype[$n] == "S") {
            $line[2] = date(DATE_FORMAT, strtotime($li_order_date[$n])) . " - " . $li_descn[$n];  //usually just the date(always unless they edited it) or wait/extra descn.
            ...
            $line[2] = $li_descn[$n];  //usually just the date(always unless they edited it) or wait/extra descn.
            ...
            if ($option_wait_extra_lines == 2) {
                for ($i = 1; $i < 14; $i++) {
                    ...
                    if ($li_waybills[$n + 1] == $li_waybills[$n] and $pos > 0) {
                        ...
                        $line[3] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_amount[$n + 1]);
        ...
        //show F.S.
        if ($option_fs_on_lines == "Y") {
            $line[4] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_fsamount[$n]);
            $line[5] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_subtot[$n]);
        }
        //show Taxes
        if ($option_tax_on_lines == "Y") {
            $line[3] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_amount[$n] + $li_fsamount[$n]);
            $line[4] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_taxes[$n]);
            $line[5] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_total[$n]);
        }
        //show both, f.s. & taxes
        if ($option_fs_on_lines == "Y" and $option_tax_on_lines == "Y") {
            $line[3] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_amount[$n]);
            $line[4] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_fsamount[$n]);
            $line[5] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_subtot[$n]);
            $line[6] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_taxes[$n]);
            $line[7] = $curSign[$c_setting['co_online_payments_currency']] . sprintf("%01.2f", $li_total[$n]);
        }
...
showrow(array("Subtotal&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                       &nbsp;&nbsp;&nbsp;", "&nbsp;&nbsp;",
    $curSign[$c_setting['co_online_payments_currency']] . $subtotal_fmt), $inputfmnd_prt);

if ($invhdr["fsamount"] != 0 && $invhdr["fsapply"] == 'Y')
    showrow(array("Fuel surcharge", "", $curSign[$c_setting['co_online_payments_currency']] . $fsamount_fmt), $inputfmnd_prt);

if ($invhdr["tax1amount"] != 0)
    showrow(array($taxname[1], "", $curSign[$c_setting['co_online_payments_currency']] . $tax1amount_fmt), $inputfmnd_prt);

if ($invhdr["tax2amount"] != 0)
    showrow(array($taxname[2], "", $curSign[$c_setting['co_online_payments_currency']] . $tax2amount_fmt), $inputfmnd_prt);

if ($invhdr["tax3amount"] != 0)
    showrow(array($taxname[3], "", $curSign[$c_setting['co_online_payments_currency']] . $tax3amount_fmt), $inputfmnd_prt);

if ($invhdr["tax4amount"] != 0)
    showrow(array($taxname[4], "", $curSign[$c_setting['co_online_payments_currency']] . $tax4amount_fmt), $inputfmnd_prt);

if ($invhdr["tax5amount"] != 0)
    showrow(array($taxname[5], "", $curSign[$c_setting['co_online_payments_currency']] . $tax5amount_fmt), $inputfmnd_prt);

$ttl = "Total";                                     //$option_stmt = "n";
$ppd = "Paid";
$due = "Due";
if ($option_stmt == "Y") {
    $ttl = "Current charges";
...
if ($invhdr["ppd_amount"] != 0) {
    showrow(array($ppd, "", $curSign[$c_setting['co_online_payments_currency']] . $ppd_amount_fmt), $inputfmnd_prt);
    showrow(array($due, "", $curSign[$c_setting['co_online_payments_currency']] . $inv_balance_fmt), $inputfmnd_prt);
}
...
if ($option_stmt == "Y") {
    ...
    showrow(array("Previous balance ", "", $curSign[$c_setting['co_online_payments_currency']] . $prev_bal_fmt), $inputfmnd_prt);
    showrow(array("Payments received", "", $curSign[$c_setting['co_online_payments_currency']] . $pymts_curr_fmt), $inputfmnd_prt);
    if ($curr_credits != 0)
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    showrow(array("Amount due       ", "", $curSign[$c_setting['co_online_payments_currency']] . $amt_due_fmt), $inputfmnd_prt);
}
