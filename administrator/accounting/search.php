...
$status = $_SESSION['status_table'];
            ...
            else $newtemp->load_template('mange_search', 1, 'accounting_menu');
            break;
                ...
                if(!empty($_POST['numOfDeliver'] ) ||  !empty($_POST['numOfOrders']))
                    {
                     ...
                     //   var_dump($res);
                      if ($_SESSION['user_type'] == 'accounting' || $_SESSION['user_type'] == 'smallaccounting' )  $newtemp->load_template('out_order_view', 1, 'accounting_menu');
                      else $newtemp->load_template('out_order_view', 1, 'accounting_menu');
                   
                    }else{
                
                      if ($_SESSION['user_type'] == 'accounting' || $_SESSION['user_type'] == 'smallaccounting' )  $newtemp->load_template('searchDeliveries',4);
                      else $newtemp->load_template('searchDeliveries',1, 'accounting_menu');
                    }
            break;
