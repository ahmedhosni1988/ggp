...
//////////end  ahme reda///////////
        case 'mange_add_account':
            ...
            $newtemp->load_template('manage_add_supplier', 5);
            break;
            ...
            case 'account_details':
                if (isset($_GET['account_id']) && $_GET['account_id'] != '') {
                    if (isset($_POST["account_company"])) {
                        ...
                        $locations = $account->get_categorys_type("Location");
                        $usertype = $account->get_categorys_type("usertype");
                        $newtemp->load_template('supplier_details', 5);
                    }
                } else {
...
//var_dump($coloums);
            ...
            $grid_sql = $console->get_grid_sql($coloums, "  suppliers 
inner join billing_code on (billing_code.id = suppliers.billing_code) 
   ");
            $query = $grid_sql . " where  ".(isset($_GET['search']) ? $console->build_grid_search($coloums, $_GET['search_txt']) : '')." status = 5     ".(isset($_POST['dir']) ? "order by  ".$table_name.".".$field_name." ".$dir."" : "order by suppliers.account_id desc ");
...
// );
        ...
        default:
            ...
            $page_title = $lang[166];
            $newtemp->load_template('supplier_manage', 1, 'accounting_menu');
