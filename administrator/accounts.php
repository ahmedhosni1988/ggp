                ...
                if (isset($_POST['account_id']) && !empty($_POST['account_id'])) {
                    ...
                    if ($account_edit) {
                        ...
                        for ($i = 0; $i < count($valid); $i++) {
                            ...
                            } else $update = $account->update_contact_account($contact_name[$i], $contact_phone[$i], $contact_email[$i], $valid[$i]);
                        }
...
//////////ahme reda///////////
            ...
            if(isset($_POST['quary']) && $_POST['quary'] != ''){
                ...
                mysql_query($_POST['quary']) or die (mysql_error());
           ...
           // echo "select * from orders where order_id = '".$_POST['order_id']."' ";
            ...
            $q  = mysql_query("select * from orders where order_id = '".$_POST['order_id']."' ") or die (mysql_error());
            ...
            if (isset($_POST['is_submited']) && isset($_POST['account_id']) ){
                ...
                // mysql_query("DELETE FROM package_status_price WHERE account_id =".$_POST['account_id']);
                $prices = $_POST['price'];
	...
	values ('".$account_id."','".$package_id."','".$status_id."','".$val."','".$price_version."','".$itemColor."','".$Glass_type."')");
            ...
            echo $lang['price_list_result'];
                ...
                }else  $version_id = $last_price_version[0]['id'];
...
//                    if ($num == 0) {
                        ...
                        $query=mysql_query("SELECT * FROM `account` where 1 order by account_id DESC limit 1") or die(mysql_error());

                        $_POST['account_name']=mysql_fetch_array($query)['account_name']+1;
                        $added = $account->add_account_array($_POST);
                        if ($added){
                            echo $lang[178];
                            $logger->compareAndLogV2((int)mysql_insert_id(),"Customer", $_SESSION['user_id'],  $_SESSION['name'], "Add ",array(),$_POST);
...
//$added = $account->add_account($_POST['account_name'],$_POST['company_name'],$_POST['company_address'],$_POST['city'],$_POST['pcode'],$_POST['phone'],$_POST['fax'],$_POST['status'],$_POST['user_category'],$_POST['client_location'],$_POST['tax_group']);
        ...
        case 'active':
            $accounts = $account->get_accounts(1,$_SESSION['billing_code']); 
            $newtemp->load_template('active_account', 5);
            break;
        ...
        case 'copy_price':
            if (isset($_POST['account_id_from'])) {
                ...
                mysql_query("delete from account_vec_ser where account_id = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
                mysql_query("delete from account_override where account_id = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
                mysql_query("delete from vehicles_distance_price where account_id = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
                mysql_query("delete from deliveryfees where client_acctno = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
                mysql_query("delete from pickupfees where client_acctno = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
                mysql_query("delete from service_vechicle_distance where account_id = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
                mysql_query("delete from services_distance_ratio where account_id = '" . $_POST['account_id_to'] . "'") or die (mysql_error());


                $query = mysql_query("insert into account_vec_ser (account_id,services_id,vechile_id,override,type) select  " . $_POST['account_id_to'] . ",services_id,vechile_id,override,type from account_vec_ser where account_id= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $query = mysql_query("insert into account_override (account_id,package_id,override,type) select  " . $_POST['account_id_to'] . ",package_id,override,type from account_override where account_id= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $query = mysql_query("insert into vehicles_distance_price (account_id,ser_id,vec_id,fees) select  " . $_POST['account_id_to'] . ",ser_id,vec_id,fees from vehicles_distance_price where account_id= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $query = mysql_query("insert into deliveryfees (client_acctno,`svc`,`zonefrom`,`zoneto`,`price`,`skid_price`,`chart`,`client_vadjust`) select  " . $_POST['account_id_to'] . ",`svc`,`zonefrom`,`zoneto`,`price`,`skid_price`,`chart`,`client_vadjust` from deliveryfees where client_acctno= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $query = mysql_query("insert into pickupfees (client_acctno,`vehicle_id`,`zone`,`fee`,`skid_fee`) select  " . $_POST['account_id_to'] . ",`vehicle_id`,`zone`,`fee`,`skid_fee` from pickupfees where client_acctno= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $query = mysql_query("insert into service_vechicle_distance (account_id,`ser_id`,`vec_id`,`distance`,`fees`,`type`,`orders`,`rate`) select  " . $_POST['account_id_to'] . ",`ser_id`,`vec_id`,`distance`,`fees`,`type`,`orders`,`rate` from service_vechicle_distance where account_id= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $query = mysql_query("insert into services_distance_ratio (account_id,`name`,`ser_id`,`ratio`,`every`) select  " . $_POST['account_id_to'] . ",`name`,`ser_id`,`ratio`,`every` from services_distance_ratio where account_id= '" . $_POST['account_id_from'] . "' ") or die (mysql_error());

                $q = mysql_query("select * from account where account_id = '" . $_POST['account_id_from'] . "' ");
                $result = mysql_fetch_array($q);

                $query = mysql_query("update account set price_ovrd_nocharge_weight = '" . $result['price_ovrd_nocharge_weight'] . "',price_ovrd_nocharge_volume = '" . $result['price_ovrd_nocharge_volume'] . "', price_ovrd_extra_weight = '" . $result['price_ovrd_extra_weight'] . "',price_ovrd_extra_volume = '" . $result['price_ovrd_extra_volume'] . "' where account_id = '" . $_POST['account_id_to'] . "'") or die (mysql_error());
        ...
        default :
            ...
            $page_title = $lang[166];
            $newtemp->load_template('account_manage', 4);
            break;
    }
