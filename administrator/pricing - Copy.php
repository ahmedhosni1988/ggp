...
$add = $price->add_addservice($_POST['ch_name'],$_POST['ch_shortname'],$_POST['ch_fee']);
if ($add){
echo $lang[313];
}else{
echo $lang[314];
...
$edit = $price->edit_addservice($_POST['ch_name'],$_POST['ch_shortname'],$_POST['ch_fee'],$_POST['edit_check_id']);
if ($edit){
echo $lang[313];
}else{
echo $lang[314];
...
if (isset($_POST['is_submited'])){
	...
	mysql_query("TRUNCATE package_status_price");
	$prices = $_POST['price'];
...
foreach ($prices as $package_id => $value) {
	...
	if($val != '') mysql_query("insert into package_status_price (	package_id,status_id,	fee) values ('".$package_id."','".$status_id."','".$val."')");
...
$price_values = $price->get_pac_status_pricelist();
...
$newtemp->load_template('price_option_mange',4);
break;
    ...
    switch ($action) {
        ...
        case 'price_pac':
            ...
            if (isset($_POST['add_pac'])) {
                ...
                $editold = $package->get_packages($_POST['edit_pac_id']);
                $edit = $package->edit_package($_POST['edit_pac_id'], $_POST['pac_name'], $_POST['pac_name_short']);
                if ($edit) {
                    echo $lang[313];
              $logger->compareAndLogV2((int)$_POST['edit_pac_id'],"Items", $_SESSION['user_id'],  $_SESSION['name'], "ُEdit_price ",object_to_array($editold),$_POST);
                ...
                $deteils = $package->get_packages($_GET['id']);
                $deteils   =object_to_array($deteils);
                $del = $package->delete_package($_GET['id']);
                if ($del) {
                    echo $lang[319];
                    $logger->compareAndLogV2((int)$_GET['id'],"Items", $_SESSION['user_id'],  $_SESSION['name'], "ُDelete_price ",array(),$deteils);
                   //var_dump(object_to_array($deteils));
